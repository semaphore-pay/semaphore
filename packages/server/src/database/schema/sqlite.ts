...
// Prefix all tables to prevent collisions in the user's database
const sqliteTable = sqliteTableCreator((name) => `semaphore_pay_${name}`);

// Standardize SQLite data types using Drizzle modes
...
export const customer = sqliteTable(
  "customer",
  {
    ...
    metadata: text("metadata", { mode: "json" }).$type<Record<
      ...
      string
    > | null>(),
    nombaCustomerId: text("nomba_customer_id"),
...
export const paymentMethod = sqliteTable(
  "payment_method",
  {
    ...
    nombaTokenId: text("nomba_token_id").notNull(),
    type: text("type"),
    brand: text("brand"),
    last4: text("last4"),
...
export const productFeature = sqliteTable(
  "product_feature",
  {
...
export const plan = sqliteTable(
  "plan",
  {
    ...
    features: text("features", { mode: "json" }).$type<PlanFeatureInput[]>().notNull().default([]),
    badge: text("badge"),
...
export const subscription = sqliteTable(
  "subscription",
  {
    ...
    planId: text("plan_id")
      .notNull()
      .references(() => plan.id),
    productInternalId: text("product_internal_id").references(() => product.internalId),
...
export const entitlement = sqliteTable(
  "entitlement",
  {
    ...
    sourceType: text("source_type").notNull().default("subscription"),
...
export const productPurchase = sqliteTable(
  "product_purchase",
  {
    id: text("id").primaryKey(),
    collectionId: text("collection_id")
...
export const webhookEvent = sqliteTable(
  ...
  {
    id: text("id").primaryKey(),
    nombaEventId: text("nomba_event_id").notNull(),
